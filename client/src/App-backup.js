...
import { initWeb3Basic } from './web3-basic';
...
function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const { contract, accounts } = await initWeb3Basic();
  ...
  const handleManualConnect = ({ web3, contract, accounts }) => {
    setContract(contract);
    setAccount(accounts[0]);
    setWeb3Ready(true);
  ...
  const handleRetryConnection = async () => {
    console.log('=== MANUAL RETRY CONNECTION STARTED ===');
    setLoading(true);
    setError('');
    
    try {
      // Clear any existing web3 instances
      if (window.web3) {
        delete window.web3;
      }
      ...
      console.log('Connection successful, setting state...');
      setContract(contract);
      setAccount(accounts[0]);
      setWeb3Ready(true);
      setNotification('Connected to MetaMask successfully!');
      ...
      try {
        ...
        console.log('Connection test successful - book count:', bookCount);
      } catch (testError) {
      ...
      if (e.message.includes('pending')) {
        setError('MetaMask permission request is pending. Please check MetaMask and approve the connection, then retry.');
      } else if (e.message.includes('User rejected')) {
        setError('Connection rejected by user. Please try again and approve the connection in MetaMask.');
      } else if (e.message.includes('switch to Ganache')) {
        setError('Please switch to Ganache network (Chain ID: 5777) in MetaMask, then retry.');
      } else if (e.message.includes('not found')) {
        setError('MetaMask not found. Please install MetaMask extension and refresh the page.');
      } else {
        setError(`Connection failed: ${e.message}. Please check MetaMask and try again.`);
      }
    }
    ...
    try {
      ...
      if (contract && account && web3Ready) {
        try {
          ...
          // Get transaction details for more information
          const txReceipt = await window.web3.eth.getTransactionReceipt(blockchainTxHash);
          console.log('Transaction Receipt:', txReceipt);
          
        } catch (blockchainError) {
          console.log('Blockchain transaction failed, using localStorage:', blockchainError);
        }
      }
      ...
      const newReceipt = {
        ...
        status: 'Confirmed',
        image: book.image,
        txHash: blockchainTxHash || ('0x' + Math.random().toString(16).substr(2, 64)),
        blockNumber: blockNumber || Math.floor(Math.random() * 1000) + 1,
        gasUsed: gasUsed || Math.floor(Math.random() * 50000) + 21000,
        timestamp: issueDate,
    ...
    try {
      ...
      if (contract && account && web3Ready) {
        try {
          ...
          console.log('Blockchain return transaction failed, using localStorage:', blockchainError);
        }
      }
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      <Box sx={{ flex: 1, p: 3 }}>
        ...
        {!showLogin && !showRegister && selectedSection === 'History' && (
          <History receipts={receipts} onDownload={handleDownloadReceipt} />
        )}
